...
extern void gen_var_or_field ANSI_PROTO((
    symbol_t *sym, int tabpos, int colonpos, int import, 
    char * rename, int hidden));

extern void print_value ANSI_PROTO(( host_int_t val, int base ));
extern void print_fp_value ANSI_PROTO(( host_float_t val ));
extern void print_value ANSI_PROTO((host_int_t val, int base));
extern void print_position ANSI_PROTO((file_pos_t pos));

extern char * string_name ANSI_PROTO((int is_wide));
extern void   gen_char_array(char* name,
			     char* val,
			     boolean is_wide_string,
			     boolean is_const);

extern int should_import ANSI_PROTO((void));
extern void print_string_value( char* val,
 			        int expected_len,
			        boolean c_string );
extern void print_char_value( int val );

extern void subtype_decl ANSI_PROTO((
    char *subtype_name, 
    char *package_name, 
    char *type_name,
    int indent,
...
extern void interface_c ANSI_PROTO((symbol_t *sym, int indent));

extern void init_predef_names ANSI_PROTO((void));
