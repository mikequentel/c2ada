...
static void
new_unchecked_cvt(symbol_pt   sym,
		  typeinfo_pt from_type,
		  typeinfo_pt to_type,
		  int         unit,
		  boolean     in_spec  )
{
    ...
    uc->next      = 0;
    if (!unchecked_cvt_list) unchecked_cvt_list = uc;
    if (unchecked_cvt_tail) unchecked_cvt_tail->next = uc;
    unchecked_cvt_tail = uc;

}

void
gen_unchecked_conversion_funcs( int unit,
			        gen_unchecked_conversion_func_pt genf)
{
    unchecked_cvt_pt uc;
			  ...
			  boolean     in_spec )
{
	...
	if (cvt->unit == unit &&
	    equal_types(from_type, cvt->from_type) &&
...
node_pt
stdarg_empty_node( file_pos_t pos )
{
    if (!stdarg_empty_sym) {
...
void
unit_uses_type( unit_type_usage_pt usage,  unit_n unit, typeinfo_pt type )
{
    ...
    if (!usage->unitDict) init_unit_dict();
	...
	/* Py_DECREF(item); */
	if (same_ada_type( type_item, type )) return;
    }
...
void
use_stdarg_concat( unit_n unit, typeinfo_pt type )
{
...
static void
gen_unit_type_usages(unit_type_usage_pt          usage,
		     unit_n                      unit,
		     gen_unit_type_usage_func_pt f     )
{
    typeinfo_pt type_item;
    int i;
...
void
gen_stdarg_concat_funcs( unit_n unit, gen_stdarg_concat_func_pt f )
{
